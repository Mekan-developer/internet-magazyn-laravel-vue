        ...
        <div class="container">
          ...
          <div class="container mt-5">
            ...
            <form>
              ...
              <!-- Submit Button -->
              <button @click.prevent="storeOrder" type="submit" class="btn btn-primary">
                Submit
              </button>
            </form>
          </div>
...
export default {
  ...
  methods: {
    ...
    getCartProducts() {
      ...
      // Берём только id
      let product_ids = carts.map((cart) => cart.id)
      api.post('/cart-products', { product_ids: product_ids }).then((res) => {
        this.products = res.data.data
        // merge qty from cart into products and calculate subtotal
        this.products.map((product) => {
          let cartItem = carts.find((item) => item.id === product.id)
          let qty = cartItem ? cartItem.qty : 0
          this.totalPrice = Number(this.totalPrice) + Number(product.price) * Number(qty)
        })
      })
    },
  },
